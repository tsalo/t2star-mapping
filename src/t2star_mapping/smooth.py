...
def _grid_indices(shape: tuple[int, int, int]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
	nx, ny, nz = shape
...
def _design_matrix(x: np.ndarray, y: np.ndarray, z: np.ndarray, terms: np.ndarray) -> np.ndarray:
	n = x.size
	nt = terms.shape[0]
...
def smooth_and_gradZ_polyfit3d(freq_3d: np.ndarray, mask_3d: np.ndarray, opts: SmoothOptions) -> tuple[np.ndarray, np.ndarray]:
	...
	def design_matrix_dz(xv: np.ndarray, yv: np.ndarray, zv: np.ndarray, t: np.ndarray) -> np.ndarray:
		nt = t.shape[0]
	...
	for ix, x0 in enumerate(xi):
		for iy, y0 in enumerate(yi):
			for iz, z0 in enumerate(zi):
				freq_smooth[x0 : min(x0 + dx, nx), y0 : min(y0 + dy, ny), z0 : min(z0 + dz, nz)] = datafit[ix, iy, iz]
				gradZ[x0 : min(x0 + dx, nx), y0 : min(y0 + dy, ny), z0 : min(z0 + dz, nz)] = datafit_dz[ix, iy, iz]
